const title = "The 18 Mistakes That Kill Startups";
const korean_title = "스타트업을 망하게 하는 18가지 실수";
const date = "October 2006";

const essay_data = [
    {
        english: `October 2006

In the Q & A period after a recent talk, someone asked what made startups fail. After standing there gaping for a few seconds I realized this was kind of a trick question. It's equivalent to asking how to make a startup succeed — if you avoid every cause of failure, you succeed — and that's too big a question to answer on the fly.`,
        korean: `2006년 10월

최근 한 강연 후 질의응답 시간에 누군가가 스타트업이 실패하는 원인이 무엇인지 물었습니다. 잠시 입을 벌린 채 멍하니 서 있다가 이것이 일종의 함정 질문이라는 것을 깨달았습니다. 이는 스타트업을 성공시키는 방법을 묻는 것과 같습니다. 모든 실패 원인을 피하면 성공하는 것이니까요. 그리고 그것은 즉석에서 답하기에는 너무 큰 질문입니다.`,
    },
    {
        english: `Afterwards I realized it could be helpful to look at the problem from this direction. If you have a list of all the things you shouldn't do, you can turn that into a recipe for succeeding just by negating. And this form of list may be more useful in practice. It's easier to catch yourself doing something you shouldn't than always to remember to do something you should. [1](This is not a complete list of the causes of failure, just those you can control. There are also several you can't, notably ineptitude and bad luck.)`,
        korean: `그 후 저는 이 방향에서 문제를 살펴보는 것이 도움이 될 수 있다는 것을 깨달았습니다. 만약 하지 말아야 할 모든 일들의 목록이 있다면, 그것들을 부정함으로써 성공을 위한 비법으로 바꿀 수 있습니다. 그리고 이런 형태의 목록은 실제 상황에서 더 유용할 수 있습니다. 항상 해야 할 일을 기억하는 것보다 하지 말아야 할 일을 하고 있는 자신을 발견하는 것이 더 쉽기 때문입니다. [1](이것은 실패 원인의 전체 목록이 아니라, 당신이 통제할 수 있는 것들만 모아놓은 것입니다. 무능함이나 불운과 같이 통제할 수 없는 몇 가지 원인도 있습니다.)`,
    },
    {
        english: `In a sense there's just one mistake that kills startups: not making something users want. If you make something users want, you'll probably be fine, whatever else you do or don't do. And if you don't make something users want, then you're dead, whatever else you do or don't do. So really this is a list of 18 things that cause startups not to make something users want. Nearly all failure funnels through that.`,
        korean: `어떤 의미에서 스타트업을 망하게 하는 실수는 단 하나, 바로 사용자가 원하는 것을 만들지 않는 것입니다. 만약 사용자가 원하는 것을 만든다면, 다른 어떤 일을 하든 아마 괜찮을 것입니다. 그리고 사용자가 원하는 것을 만들지 않는다면, 다른 어떤 일을 하든 하지 않든 당신은 끝장입니다. 그래서 이것은 사실상 스타트업이 사용자가 원하는 것을 만들지 못하게 하는 18가지 원인 목록입니다. 거의 모든 실패가 그곳을 통해 빠져나갑니다.`,
    },
    {
        english: `1. Single Founder

Have you ever noticed how few successful startups were founded by just one person? Even companies you think of as having one founder, like Oracle, usually turn out to have more. It seems unlikely this is a coincidence.

What's wrong with having one founder? To start with, it's a vote of no confidence. It probably means the founder couldn't talk any of his friends into starting the company with him. That's pretty alarming, because his friends are the ones who know him best.

But even if the founder's friends were all wrong and the company is a good bet, he's still at a disadvantage. Starting a startup is too hard for one person. Even if you could do all the work yourself, you need colleagues to brainstorm with, to talk you out of stupid decisions, and to cheer you up when things go wrong.

The last one might be the most important. The low points in a startup are so low that few could bear them alone. When you have multiple founders, esprit de corps binds them together in a way that seems to violate conservation laws. Each thinks "I can't let my friends down." This is one of the most powerful forces in human nature, and it's missing when there's just one founder.`,
        korean: `1. 단독 창업자

성공한 스타트업 중 단 한 사람이 설립한 경우가 얼마나 적은지 눈치채신 적 있나요? 오라클처럼 단독 창업자로 생각되는 회사들조차 실제로는 더 많은 공동 창업자가 있는 경우가 많습니다. 이것이 우연의 일치일 가능성은 낮아 보입니다.

단독 창업의 문제점은 무엇일까요? 우선, 그것은 불신임 투표와 같습니다. 아마도 창업자가 친구들 중 누구도 함께 회사를 시작하도록 설득하지 못했다는 의미일 것입니다. 친구들이야말로 그를 가장 잘 아는 사람들이기 때문에 이는 매우 놀라운 일입니다.

하지만 설령 창업자의 친구들이 모두 틀렸고 회사가 좋은 투자처라고 해도, 그는 여전히 불리한 입장에 있습니다. 스타트업을 시작하는 것은 한 사람에게는 너무 힘든 일입니다. 모든 일을 혼자 다 할 수 있다고 해도, 함께 브레인스토밍하고, 어리석은 결정을 말리고, 일이 잘못될 때 격려해 줄 동료가 필요합니다.

마지막 요소가 가장 중요할지도 모릅니다. 스타트업의 암흑기는 너무나 힘들어서 혼자서는 감당하기 어려운 경우가 많습니다. 여러 명의 창업자가 있을 때는, 동료애(esprit de corps)가 마치 보존 법칙을 위반하는 것처럼 그들을 하나로 묶어줍니다. 각자는 "내 친구들을 실망시킬 수 없어."라고 생각합니다. 이것은 인간 본성 중 가장 강력한 힘 중 하나이며, 단독 창업자에게는 이것이 없습니다.`,
    },
    {
        english: `2. Bad Location

Startups prosper in some places and not others. Silicon Valley dominates, then Boston, then Seattle, Austin, Denver, and New York. After that there's not much. Even in New York the number of startups per capita is probably a 20th of what it is in Silicon Valley. In towns like Houston and Chicago and Detroit it's too small to measure.

Why is the falloff so sharp? Probably for the same reason it is in other industries. What's the sixth largest fashion center in the US? The sixth largest center for oil, or finance, or publishing? Whatever they are they're probably so far from the top that it would be misleading even to call them centers.

It's an interesting question why cities become startup hubs, but the reason startups prosper in them is probably the same as it is for any industry: that's where the experts are. Standards are higher; people are more sympathetic to what you're doing; the kind of people you want to hire want to live there; supporting industries are there; the people you run into in chance meetings are in the same business. Who knows exactly how these factors combine to boost startups in Silicon Valley and squish them in Detroit, but it's clear they do from the number of startups per capita in each.`,
        korean: `2. 잘못된 위치

스타트업은 특정 장소에서는 번창하고 다른 곳에서는 그렇지 못합니다. 실리콘 밸리가 지배적이고, 그 다음이 보스턴, 시애틀, 오스틴, 덴버, 뉴욕 순입니다. 그 외에는 그다지 많지 않습니다. 뉴욕조차도 1인당 스타트업 수는 아마 실리콘 밸리의 20분의 1에 불과할 것입니다. 휴스턴, 시카고, 디트로이트 같은 도시에서는 측정하기 어려울 정도로 적습니다.

왜 이렇게 급격한 감소가 나타날까요? 아마도 다른 산업에서도 같은 이유일 것입니다. 미국에서 여섯 번째로 큰 패션 중심지는 어디일까요? 석유, 금융, 또는 출판 분야에서 여섯 번째로 큰 중심지는요? 그곳이 어디든 아마 정상과는 거리가 멀어서 중심지라고 부르기조차 오해의 소지가 있을 것입니다.

도시가 스타트업 허브가 되는 이유는 흥미로운 질문이지만, 스타트업이 그곳에서 번창하는 이유는 아마도 다른 산업과 마찬가지일 것입니다. 바로 전문가들이 그곳에 있기 때문입니다. 기준이 더 높고, 사람들이 당신이 하는 일에 더 공감하며, 고용하고 싶은 종류의 사람들이 그곳에 살고 싶어 하고, 지원 산업이 존재하며, 우연히 만나는 사람들이 같은 업계에 종사합니다. 이러한 요소들이 어떻게 결합하여 실리콘 밸리에서는 스타트업을 활성화하고 디트로이트에서는 위축시키는지 정확히는 알 수 없지만, 각 도시의 1인당 스타트업 수를 보면 그 효과는 분명합니다.`,
    },
    {
        english: `3. Marginal Niche

Most of the groups that apply to Y Combinator suffer from a common problem: choosing a small, obscure niche in the hope of avoiding competition.

If you watch little kids playing sports, you notice that below a certain age they're afraid of the ball. When the ball comes near them their instinct is to avoid it. I didn't make a lot of catches as an eight year old outfielder, because whenever a fly ball came my way, I used to close my eyes and hold my glove up more for protection than in the hope of catching it.

Choosing a marginal project is the startup equivalent of my eight year old strategy for dealing with fly balls. If you make anything good, you're going to have competitors, so you may as well face that. You can only avoid competition by avoiding good ideas.

I think this shrinking from big problems is mostly unconscious. It's not that people think of grand ideas but decide to pursue smaller ones because they seem safer. Your unconscious won't even let you think of grand ideas. So the solution may be to think about ideas without involving yourself. What would be a great idea for someone else to do as a startup?`,
        korean: `3. 주변 틈새시장

Y Combinator에 지원하는 대부분의 그룹은 경쟁을 피하기 위해 작고 모호한 틈새시장을 선택하는 공통적인 문제를 겪습니다.

어린 아이들이 운동하는 것을 보면, 특정 연령 이하에서는 공을 무서워한다는 것을 알 수 있습니다. 공이 가까이 오면 본능적으로 피하려고 합니다. 저는 8살 때 외야수로서 공을 많이 잡지 못했는데, 뜬공이 제 쪽으로 날아올 때마다 눈을 감고 잡으려는 희망보다는 보호를 위해 글러브를 올리곤 했습니다.

주변적인 프로젝트를 선택하는 것은 뜬공을 대처하는 저의 8살짜리 전략과 같은 스타트업의 행태입니다. 좋은 것을 만들면 경쟁자가 생기기 마련이므로, 그 점을 직시하는 것이 좋습니다. 좋은 아이디어를 피해야만 경쟁을 피할 수 있습니다.

I think this shrinking from big problems is mostly unconscious. It's not that people think of grand ideas but decide to pursue smaller ones because they seem safer. Your unconscious won't even let you think of grand ideas. So the solution may be to think about ideas without involving yourself. What would be a great idea for someone else to do as a startup?`,
    },
    {
        english: `4. Derivative Idea

Many of the applications we get are imitations of some existing company. That's one source of ideas, but not the best. If you look at the origins of successful startups, few were started in imitation of some other startup. Where did they get their ideas? Usually from some specific, unsolved problem the founders identified.

Our startup made software for making online stores. When we started it, there wasn't any; the few sites you could order from were hand-made at great expense by web consultants. We knew that if online shopping ever took off, these sites would have to be generated by software, so we wrote some. Pretty straightforward.

It seems like the best problems to solve are ones that affect you personally. Apple happened because Steve Wozniak wanted a computer, Google because Larry and Sergey couldn't find stuff online, Hotmail because Sabeer Bhatia and Jack Smith couldn't exchange email at work.

So instead of copying the Facebook, with some variation that the Facebook rightly ignored, look for ideas from the other direction. Instead of starting from companies and working back to the problems they solved, look for problems and imagine the company that might solve them. [2](Ironically, one variant of the Facebook that might work is a facebook exclusively for college students.) What do people complain about? What do you wish there was?`,
        korean: `4. 파생적인 아이디어

우리가 받는 지원서 중 다수는 기존 회사를 모방한 것입니다. 그것도 아이디어의 한 가지 원천이지만, 최선은 아닙니다. 성공한 스타트업의 기원을 살펴보면, 다른 스타트업을 모방하여 시작된 경우는 거의 없습니다. 그들은 어디서 아이디어를 얻었을까요? 보통 창업자들이 발견한 구체적이고 해결되지 않은 문제에서 비롯됩니다.

우리 스타트업은 온라인 상점을 만드는 소프트웨어를 제작했습니다. 우리가 시작했을 때는 그런 것이 없었습니다. 주문할 수 있는 몇 안 되는 사이트들은 웹 컨설턴트들이 엄청난 비용을 들여 수작업으로 만든 것이었습니다. 우리는 온라인 쇼핑이 활성화되면 이러한 사이트들이 소프트웨어로 생성되어야 한다는 것을 알았고, 그래서 소프트웨어를 만들었습니다. 매우 간단명료했죠.

해결하기 가장 좋은 문제는 개인적으로 영향을 미치는 문제인 것 같습니다. 스티브 워즈니악이 컴퓨터를 원했기 때문에 애플이 탄생했고, 래리와 세르게이가 온라인에서 정보를 찾을 수 없었기 때문에 구글이, 사비르 바티아와 잭 스미스가 직장에서 이메일을 교환할 수 없었기 때문에 핫메일이 탄생했습니다.

그러니 페이스북이 당연히 무시했던 약간의 변형을 가미하여 페이스북을 모방하는 대신, 다른 방향에서 아이디어를 찾아보세요. 회사에서 시작하여 그들이 해결한 문제로 거슬러 올라가는 대신, 문제를 찾고 그것을 해결할 수 있는 회사를 상상해 보세요. [2](아이러니하게도, 효과가 있을 수 있는 페이스북의 한 가지 변형은 대학생 전용 페이스북입니다.) 사람들은 무엇에 대해 불평하나요? 무엇이 있었으면 하고 바라나요?`,
    },
    {
        english: `5. Obstinacy

In some fields the way to succeed is to have a vision of what you want to achieve, and to hold true to it no matter what setbacks you encounter. Starting startups is not one of them. The stick-to-your-vision approach works for something like winning an Olympic gold medal, where the problem is well-defined. Startups are more like science, where you need to follow the trail wherever it leads.

So don't get too attached to your original plan, because it's probably wrong. Most successful startups end up doing something different than they originally intended — often so different that it doesn't even seem like the same company. You have to be prepared to see the better idea when it arrives. And the hardest part of that is often discarding your old idea.

But openness to new ideas has to be tuned just right. Switching to a new idea every week will be equally fatal. Is there some kind of external test you can use? One is to ask whether the ideas represent some kind of progression. If in each new idea you're able to re-use most of what you built for the previous ones, then you're probably in a process that converges. Whereas if you keep restarting from scratch, that's a bad sign.

Fortunately there's someone you can ask for advice: your users. If you're thinking about turning in some new direction and your users seem excited about it, it's probably a good bet.`,
        korean: `5. 고집

어떤 분야에서는 성공하는 방법이 달성하고자 하는 비전을 갖고 어떤 좌절에 직면하든 그것을 고수하는 것입니다. 스타트업 창업은 그런 분야가 아닙니다. 비전을 고수하는 접근 방식은 올림픽 금메달 획득처럼 문제가 잘 정의된 경우에 효과적입니다. 스타트업은 과학과 더 비슷해서, 단서가 이끄는 대로 따라가야 합니다.

그러니 원래 계획에 너무 집착하지 마세요. 아마 틀렸을 가능성이 높습니다. 대부분의 성공한 스타트업은 원래 의도했던 것과 다른 일을 하게 됩니다. 너무 달라서 같은 회사처럼 보이지 않을 때도 많습니다. 더 나은 아이디어가 나타났을 때 그것을 볼 준비가 되어 있어야 합니다. 그리고 그중 가장 어려운 부분은 종종 기존 아이디어를 버리는 것입니다.

하지만 새로운 아이디어에 대한 개방성은 적절히 조율되어야 합니다. 매주 새로운 아이디어로 바꾸는 것은 마찬가지로 치명적일 것입니다. 사용할 수 있는 외부적인 테스트가 있을까요? 한 가지 방법은 아이디어들이 어떤 종류의 진행 상황을 나타내는지 묻는 것입니다. 각각의 새로운 아이디어에서 이전 아이디어를 위해 구축한 것의 대부분을 재사용할 수 있다면, 아마도 수렴하는 과정에 있는 것입니다. 반면에 계속해서 처음부터 다시 시작한다면 그것은 나쁜 징조입니다.

다행히도 조언을 구할 수 있는 사람이 있습니다. 바로 당신의 사용자입니다. 새로운 방향으로 전환하는 것을 고려하고 있고 사용자들이 그것에 대해 흥분하는 것처럼 보인다면, 그것은 아마 좋은 선택일 것입니다.`,
    },
    {
        english: `6. Hiring Bad Programmers

I forgot to include this in the early versions of the list, because nearly all the founders I know are programmers. This is not a serious problem for them. They might accidentally hire someone bad, but it's not going to kill the company. In a pinch they can do whatever's required themselves.

But when I think about what killed most of the startups in the e-commerce business back in the 90s, it was bad programmers. A lot of those companies were started by business guys who thought the way startups worked was that you had some clever idea and then hired programmers to implement it. That's actually much harder than it sounds — almost impossibly hard in fact — because business guys can't tell which are the good programmers. They don't even get a shot at the best ones, because no one really good wants a job implementing the vision of a business guy.

In practice what happens is that the business guys choose people they think are good programmers (it says here on his resume that he's a Microsoft Certified Developer) but who aren't. Then they're mystified to find that their startup lumbers along like a World War II bomber while their competitors scream past like jet fighters. This kind of startup is in the same position as a big company, but without the advantages.

So how do you pick good programmers if you're not a programmer? I don't think there's an answer. I was about to say you'd have to find a good programmer to help you hire people. But if you can't recognize good programmers, how would you even do that?`,
        korean: `6. 실력 없는 프로그래머 고용

제가 아는 거의 모든 창업자가 프로그래머이기 때문에 목록의 초기 버전에는 이 항목을 포함하는 것을 잊었습니다. 이것은 그들에게 심각한 문제가 아닙니다. 실수로 실력 없는 사람을 고용할 수도 있지만, 회사를 망하게 하지는 않을 것입니다. 급한 상황에서는 필요한 모든 일을 스스로 할 수 있습니다.

하지만 90년대 전자상거래 업계의 스타트업 대부분을 망하게 한 것이 무엇인지 생각해보면, 그것은 실력 없는 프로그래머들이었습니다. 그 회사들 중 다수는 기발한 아이디어를 떠올린 다음 프로그래머를 고용하여 구현하면 된다고 생각하는 사업가들이 시작했습니다. 그것은 실제로는 생각보다 훨씬 어렵습니다. 사실상 거의 불가능할 정도로 어렵습니다. 왜냐하면 사업가들은 누가 좋은 프로그래머인지 구분할 수 없기 때문입니다. 그들은 최고의 프로그래머들을 만날 기회조차 얻지 못합니다. 정말 뛰어난 사람 중 누구도 사업가의 비전을 구현하는 일을 원하지 않기 때문입니다.`,
    },
    {
        english: `7. Choosing the Wrong Platform

A related problem (since it tends to be done by bad programmers) is choosing the wrong platform. For example, I think a lot of startups during the Bubble killed themselves by deciding to build server-based applications on Windows. Hotmail was still running on FreeBSD for years after Microsoft bought it, presumably because Windows couldn't handle the load. If Hotmail's founders had chosen to use Windows, they would have been swamped.

PayPal only just dodged this bullet. After they merged with X.com, the new CEO wanted to switch to Windows — even after PayPal cofounder Max Levchin showed that their software scaled only 1% as well on Windows as Unix. Fortunately for PayPal they switched CEOs instead.

Platform is a vague word. It could mean an operating system, or a programming language, or a "framework" built on top of a programming language. It implies something that both supports and limits, like the foundation of a house.

The scary thing about platforms is that there are always some that seem to outsiders to be fine, responsible choices and yet, like Windows in the 90s, will destroy you if you choose them. Java applets were probably the most spectacular example. This was supposed to be the new way of delivering applications. Presumably it killed just about 100% of the startups who believed that.

How do you pick the right platforms? The usual way is to hire good programmers and let them choose. But there is a trick you could use if you're not a programmer: visit a top computer science department and see what they use in research projects.`,
        korean: `7. 잘못된 플랫폼 선택

관련된 문제(주로 실력 없는 프로그래머들이 저지르는 경향이 있기 때문에)는 잘못된 플랫폼을 선택하는 것입니다. 예를 들어, 버블 시대의 많은 스타트업들이 윈도우 기반 서버 애플리케이션을 구축하기로 결정함으로써 자멸했다고 생각합니다. 핫메일은 마이크로소프트에 인수된 후에도 몇 년 동안 FreeBSD에서 실행되었는데, 이는 아마도 윈도우가 부하를 감당할 수 없었기 때문일 것입니다. 만약 핫메일 창업자들이 윈도우를 사용하기로 선택했다면, 그들은 감당할 수 없었을 것입니다.

페이팔은 이 총알을 간신히 피했습니다. X.com과 합병한 후, 새로운 CEO는 윈도우로 전환하기를 원했습니다. 페이팔 공동 창업자인 맥스 레브친이 그들의 소프트웨어가 유닉스에서보다 윈도우에서 확장성이 1%밖에 되지 않는다는 것을 보여준 후에도 말입니다. 다행히 페이팔은 대신 CEO를 교체했습니다.

플랫폼은 모호한 단어입니다. 운영 체제, 프로그래밍 언어, 또는 프로그래밍 언어 위에 구축된 "프레임워크"를 의미할 수 있습니다. 그것은 집의 기초처럼 지지하면서 동시에 제한하는 것을 의미합니다.

플랫폼에 대해 무서운 점은 외부인에게는 괜찮고 책임감 있는 선택처럼 보이지만, 90년대의 윈도우처럼 선택하면 당신을 파괴할 플랫폼이 항상 있다는 것입니다. 자바 애플릿이 아마 가장 극적인 예일 것입니다. 이것은 애플리케이션을 제공하는 새로운 방법으로 여겨졌습니다. 아마도 그것을 믿었던 스타트업의 거의 100%를 죽였을 것입니다.

올바른 플랫폼을 어떻게 선택할까요? 일반적인 방법은 좋은 프로그래머를 고용하고 그들이 선택하게 하는 것입니다. 하지만 프로그래머가 아니라면 사용할 수 있는 비결이 있습니다. 최고의 컴퓨터 과학과를 방문하여 연구 프로젝트에서 무엇을 사용하는지 확인하는 것입니다.`,
    },
    {
        english: `8. Slowness in Launching

Companies of all sizes have a hard time getting software done. It's intrinsic to the medium; software is always 85% done. It takes an effort of will to push through this and get something released to users. [3](Steve Jobs tried to motivate people by saying "Real artists ship." This is a fine sentence, but unfortunately not true. Many famous works of art are unfinished. It's true in fields that have hard deadlines, like architecture and filmmaking, but even there people tend to be tweaking stuff till it's yanked out of their hands.)

Startups make all kinds of excuses for delaying their launch. Most are equivalent to the ones people use for procrastinating in everyday life. There's something that needs to happen first. Maybe. But if the software were 100% finished and ready to launch at the push of a button, would they still be waiting?

One reason to launch quickly is that it forces you to actually finish some quantum of work. Nothing is truly finished till it's released; you can see that from the rush of work that's always involved in releasing anything, no matter how finished you thought it was. The other reason you need to launch is that it's only by bouncing your idea off users that you fully understand it.

Several distinct problems manifest themselves as delays in launching: working too slowly; not truly understanding the problem; fear of having to deal with users; fear of being judged; working on too many different things; excessive perfectionism. Fortunately you can combat all of them by the simple expedient of forcing yourself to launch something fairly quickly.`,
        korean: `8. 느린 출시

모든 규모의 회사가 소프트웨어 완성을 어려워합니다. 그것은 매체의 본질적인 특성입니다. 소프트웨어는 항상 85%만 완성된 상태입니다. 이것을 밀어붙여 사용자에게 무언가를 출시하려면 의지력이 필요합니다. [3](스티브 잡스는 "진정한 예술가는 결과물을 내놓는다"라는 말로 사람들에게 동기를 부여하려고 했습니다. 멋진 문장이지만, 안타깝게도 사실이 아닙니다. 많은 유명한 예술 작품이 미완성 상태입니다. 건축이나 영화 제작처럼 마감 기한이 엄격한 분야에서는 사실일 수 있지만, 그곳에서조차 사람들은 결과물을 빼앗기기 직전까지 계속해서 수정하는 경향이 있습니다.)

스타트업은 출시를 늦추는 온갖 변명을 합니다. 대부분은 사람들이 일상생활에서 꾸물거릴 때 사용하는 변명과 비슷합니다. 먼저 해야 할 일이 있다는 것입니다. 그럴 수도 있습니다. 하지만 소프트웨어가 100% 완성되어 버튼 하나만 누르면 출시할 수 있는 상태라면, 그들은 여전히 기다리고 있을까요?

빨리 출시해야 하는 한 가지 이유는 그것이 실제로 일정량의 작업을 완료하도록 강제하기 때문입니다. 출시될 때까지는 아무것도 진정으로 끝난 것이 아닙니다. 아무리 끝났다고 생각했더라도 무언가를 출시하는 데 항상 수반되는 작업의 몰림을 보면 알 수 있습니다. 출시해야 하는 또 다른 이유는 아이디어를 사용자에게 시험해봄으로써만 그것을 완전히 이해할 수 있기 때문입니다.

출시 지연으로 나타나는 몇 가지 뚜렷한 문제점들이 있습니다. 너무 느리게 작업하는 것, 문제를 제대로 이해하지 못하는 것, 사용자를 상대해야 하는 것에 대한 두려움, 평가받는 것에 대한 두려움, 너무 많은 다른 일에 매달리는 것, 과도한 완벽주의 등입니다. 다행히도 자신을 강제하여 무언가를 상당히 빨리 출시하는 간단한 방법으로 이 모든 문제에 대처할 수 있습니다.`,
    },
    {
        english: `9. Launching Too Early

Launching too slowly has probably killed a hundred times more startups than launching too fast, but it is possible to launch too fast. The danger here is that you ruin your reputation. You launch something, the early adopters try it out, and if it's no good they may never come back.

So what's the minimum you need to launch? We suggest startups think about what they plan to do, identify a core that's both (a) useful on its own and (b) something that can be incrementally expanded into the whole project, and then get that done as soon as possible.

This is the same approach I (and many other programmers) use for writing software. Think about the overall goal, then start by writing the smallest subset of it that does anything useful. If it's a subset, you'll have to write it anyway, so in the worst case you won't be wasting your time. But more likely you'll find that implementing a working subset is both good for morale and helps you see more clearly what the rest should do.

The early adopters you need to impress are fairly tolerant. They don't expect a newly launched product to do everything; it just has to do something.`,
        korean: `9. 너무 이른 출시

너무 느린 출시는 너무 빠른 출시보다 아마 백 배는 더 많은 스타트업을 죽였을 것이지만, 너무 빨리 출시하는 것도 가능합니다. 여기서 위험은 평판을 망칠 수 있다는 것입니다. 무언가를 출시하고, 얼리 어답터들이 그것을 시험해보고, 만약 좋지 않다면 그들은 다시는 돌아오지 않을 수도 있습니다.

그렇다면 출시하는 데 필요한 최소한의 것은 무엇일까요? 우리는 스타트업이 무엇을 할 계획인지 생각하고, (a) 그 자체로 유용하고 (b) 전체 프로젝트로 점진적으로 확장될 수 있는 핵심을 파악한 다음, 가능한 한 빨리 그것을 완료하도록 제안합니다.

이것은 저(그리고 다른 많은 프로그래머들)가 소프트웨어를 작성할 때 사용하는 것과 같은 접근 방식입니다. 전체 목표에 대해 생각한 다음, 유용한 작업을 수행하는 가장 작은 하위 집합을 작성하는 것부터 시작합니다. 그것이 하위 집합이라면 어쨌든 작성해야 하므로 최악의 경우에도 시간을 낭비하지는 않을 것입니다. 하지만 작동하는 하위 집합을 구현하는 것이 사기 진작에 좋고 나머지 부분이 무엇을 해야 하는지 더 명확하게 보는 데 도움이 될 가능성이 더 큽니다.

감명을 주어야 할 얼리 어답터들은 상당히 관대합니다. 그들은 새로 출시된 제품이 모든 것을 하기를 기대하지 않습니다. 단지 무언가를 하기만 하면 됩니다.`,
    },
    {
        english: `10. Having No Specific User in Mind

You can't build things users like without understanding them. I mentioned earlier that the most successful startups seem to have begun by trying to solve a problem their founders had. Perhaps there's a rule here: perhaps you create wealth in proportion to how well you understand the problem you're solving, and the problems you understand best are your own. [4](There's probably also a second factor: startup founders tend to be at the leading edge of technology, so problems they face are probably especially valuable.)

That's just a theory. What's not a theory is the converse: if you're trying to solve problems you don't understand, you're hosed.

And yet a surprising number of founders seem willing to assume that someone, they're not sure exactly who, will want what they're building. Do the founders want it? No, they're not the target market. Who is? Teenagers. People interested in local events (that one is a perennial tarpit). Or "business" users. What business users? Gas stations? Movie studios? Defense contractors?

You can of course build something for users other than yourself. We did. But you should realize you're stepping into dangerous territory. You're flying on instruments, in effect, so you should (a) consciously shift gears, instead of assuming you can rely on your intuitions as you ordinarily would, and (b) look at the instruments.

In this case the instruments are the users. When designing for other people you have to be empirical. You can no longer guess what will work; you have to find users and measure their responses. So if you're going to make something for teenagers or "business" users or some other group that doesn't include you, you have to be able to talk some specific ones into using what you're making. If you can't, you're on the wrong track.`,
        korean: `10. 특정 사용자를 염두에 두지 않음

사용자를 이해하지 않고는 그들이 좋아할 만한 것을 만들 수 없습니다. 앞서 언급했듯이, 가장 성공적인 스타트업들은 창업자들이 겪었던 문제를 해결하려는 시도에서 시작된 것으로 보입니다. 아마도 여기에 규칙이 있을 것입니다. 즉, 당신이 해결하려는 문제를 얼마나 잘 이해하느냐에 비례하여 부를 창출하며, 당신이 가장 잘 이해하는 문제는 바로 당신 자신의 문제라는 것입니다. [4](아마도 두 번째 요인도 있을 것입니다: 스타트업 창업자들은 기술의 최첨단에 있는 경향이 있으므로, 그들이 직면하는 문제는 특히 가치가 있을 것입니다.)

이것은 단지 이론일 뿐입니다. 이론이 아닌 것은 그 반대입니다. 만약 당신이 이해하지 못하는 문제를 해결하려고 한다면, 당신은 망한 것입니다.

그럼에도 불구하고 놀랍게도 많은 창업자들이 정확히 누구인지는 모르지만 누군가가 자신들이 만들고 있는 것을 원할 것이라고 기꺼이 가정하는 것 같습니다. 창업자들은 그것을 원할까요? 아니요, 그들은 목표 시장이 아닙니다. 누구일까요? 십 대들. 지역 이벤트에 관심 있는 사람들(이것은 다년생의 함정입니다). 또는 "비즈니스" 사용자들. 어떤 비즈니스 사용자들일까요? 주유소? 영화 스튜디오? 방위 산업체?

물론 자신 이외의 사용자를 위해 무언가를 만들 수 있습니다. 우리는 그렇게 했습니다. 하지만 당신은 위험한 영역으로 발을 들여놓고 있다는 것을 깨달아야 합니다. 사실상 계기 비행을 하고 있는 것이므로, (a) 평소처럼 직관에 의존할 수 있다고 가정하는 대신 의식적으로 기어를 바꾸고, (b) 계기를 봐야 합니다.

이 경우 계기는 사용자입니다. 다른 사람들을 위해 디자인할 때는 경험적이어야 합니다. 더 이상 무엇이 효과가 있을지 추측할 수 없습니다. 사용자를 찾고 그들의 반응을 측정해야 합니다. 따라서 십 대나 "비즈니스" 사용자 또는 당신을 포함하지 않는 다른 그룹을 위해 무언가를 만들려면, 특정 사용자 몇 명과 이야기하여 당신이 만들고 있는 것을 사용하도록 설득할 수 있어야 합니다. 만약 그럴 수 없다면, 당신은 잘못된 길을 가고 있는 것입니다.`,
    },
    {
        english: `11. Raising Too Little Money

Most successful startups take funding at some point. Like having more than one founder, it seems a good bet statistically. How much should you take, though?

Startup funding is measured in time. Every startup that isn't profitable (meaning nearly all of them, initially) has a certain amount of time left before the money runs out and they have to stop. This is sometimes referred to as runway, as in "How much runway do you have left?" It's a good metaphor because it reminds you that when the money runs out you're going to be airborne or dead.

Too little money means not enough to get airborne. What airborne means depends on the situation. Usually you have to advance to a visibly higher level: if all you have is an idea, a working prototype; if you have a prototype, launching; if you're launched, significant growth. It depends on investors, because until you're profitable that's who you have to convince.

So if you take money from investors, you have to take enough to get to the next step, whatever that is. [5](You should take more than you think you'll need, maybe 50% to 100% more, because software takes longer to write and deals longer to close than you expect.) Fortunately you have some control over both how much you spend and what the next step is. We advise startups to set both low, initially: spend practically nothing, and make your initial goal simply to build a solid prototype. This gives you maximum flexibility.`,
        korean: `11. 너무 적은 자금 조달

대부분의 성공적인 스타트업은 어느 시점에서 자금 조달을 받습니다. 한 명 이상의 창업자를 두는 것처럼, 통계적으로 좋은 선택인 것 같습니다. 하지만 얼마나 받아야 할까요?

스타트업 자금은 시간으로 측정됩니다. 수익성이 없는(초기에는 거의 모든 스타트업을 의미) 모든 스타트업은 돈이 떨어져 멈춰야 할 때까지 일정 시간이 남아 있습니다. 이것은 때때로 "활주로가 얼마나 남았습니까?"와 같이 활주로라고 불립니다. 돈이 떨어지면 공중에 뜨거나 죽게 된다는 것을 상기시켜주기 때문에 좋은 비유입니다.

너무 적은 돈은 공중에 뜰 만큼 충분하지 않다는 것을 의미합니다. 공중에 뜬다는 것이 무엇을 의미하는지는 상황에 따라 다릅니다. 일반적으로 눈에 띄게 더 높은 수준으로 나아가야 합니다. 아이디어만 있다면 작동하는 프로토타입, 프로토타입이 있다면 출시, 출시했다면 상당한 성장과 같이 말입니다. 수익성이 생길 때까지는 투자자를 설득해야 하기 때문에 투자자에게 달려 있습니다.

따라서 투자자로부터 돈을 받는다면, 다음 단계가 무엇이든 그 단계에 도달할 만큼 충분히 받아야 합니다. [5](생각보다 더 많은 자금, 어쩌면 50%에서 100% 더 많은 자금을 확보해야 합니다. 소프트웨어 개발과 거래 성사에는 예상보다 더 오랜 시간이 걸리기 때문입니다.) 다행히도 지출액과 다음 단계가 무엇인지 모두 어느 정도 통제할 수 있습니다. 우리는 스타트업에게 처음에는 둘 다 낮게 설정하라고 조언합니다. 즉, 거의 아무것도 쓰지 말고 초기 목표를 단순히 견고한 프로토타입을 만드는 것으로 설정하는 것입니다. 이렇게 하면 유연성을 극대화할 수 있습니다.`,
    },
    {
        english: `12. Spending Too Much

It's hard to distinguish spending too much from raising too little. If you run out of money, you could say either was the cause. The only way to decide which to call it is by comparison with other startups. If you raised five million and ran out of money, you probably spent too much.

Burning through too much money is not as common as it used to be. Founders seem to have learned that lesson. Plus it keeps getting cheaper to start a startup. So as of this writing few startups spend too much. None of the ones we've funded have. (And not just because we make small investments; many have gone on to raise further rounds.)

The classic way to burn through cash is by hiring a lot of people. This bites you twice: in addition to increasing your costs, it slows you down—so money that's getting consumed faster has to last longer. Most hackers understand why that happens; Fred Brooks explained it in The Mythical Man-Month.

We have three general suggestions about hiring: (a) don't do it if you can avoid it, (b) pay people with equity rather than salary, not just to save money, but because you want the kind of people who are committed enough to prefer that, and (c) only hire people who are either going to write code or go out and get users, because those are the only things you need at first.`,
        korean: `12. 너무 많은 지출

너무 많이 지출하는 것과 너무 적게 조달하는 것을 구별하기는 어렵습니다. 돈이 떨어지면 둘 중 하나가 원인이라고 말할 수 있습니다. 어느 쪽이라고 부를지 결정하는 유일한 방법은 다른 스타트업과 비교하는 것입니다. 5백만 달러를 조달하고 돈이 떨어졌다면 아마 너무 많이 썼을 것입니다.

너무 많은 돈을 써버리는 것은 예전만큼 흔하지 않습니다. 창업자들은 그 교훈을 배운 것 같습니다. 게다가 스타트업을 시작하는 비용이 계속 저렴해지고 있습니다. 그래서 이 글을 쓰는 현재 너무 많이 지출하는 스타트업은 거의 없습니다. 우리가 자금을 지원한 스타트업 중에는 아무도 없습니다. (그리고 우리가 소액 투자를 하기 때문만은 아닙니다. 많은 스타트업이 추가 라운드를 진행했습니다.)

현금을 소진하는 전형적인 방법은 많은 사람을 고용하는 것입니다. 이것은 두 번 당신을 괴롭힙니다. 비용을 증가시키는 것 외에도 속도를 늦춥니다. 따라서 더 빨리 소비되는 돈이 더 오래 지속되어야 합니다. 대부분의 해커는 이것이 왜 발생하는지 이해합니다. 프레드 브룩스는 '신화적인 맨먼스(The Mythical Man-Month)'에서 그것을 설명했습니다.

고용에 관한 세 가지 일반적인 제안이 있습니다. (a) 피할 수 있다면 하지 마십시오. (b) 돈을 절약하기 위해서뿐만 아니라 그것을 선호할 만큼 헌신적인 사람들을 원하기 때문에 급여보다는 지분으로 사람들에게 지불하십시오. (c) 처음에는 코드 작성이나 사용자 확보 외에는 필요한 것이 없으므로 이 두 가지 중 하나를 수행할 사람들만 고용하십시오.`,
    },
    {
        english: `13. Raising Too Much Money

It's obvious how too little money could kill you, but is there such a thing as having too much?

Yes and no. The problem is not so much the money itself as what comes with it. As one VC who spoke at Y Combinator said, "Once you take several million dollars of my money, the clock is ticking." If VCs fund you, they're not going to let you just put the money in the bank and keep operating as two guys living on ramen. They want that money to go to work. [6](Since people sometimes call us VCs, I should add that we're not. VCs invest large amounts of other people's money. We invest small amounts of our own, like angel investors.) At the very least you'll move into proper office space and hire more people. That will change the atmosphere, and not entirely for the better. Now most of your people will be employees rather than founders. They won't be as committed; they'll need to be told what to do; they'll start to engage in office politics.

When you raise a lot of money, your company moves to the suburbs and has kids.

Perhaps more dangerously, once you take a lot of money it gets harder to change direction. Suppose your initial plan was to sell something to companies. After taking VC money you hire a sales force to do that. What happens now if you realize you should be making this for consumers instead of businesses? That's a completely different kind of selling. What happens, in practice, is that you don't realize that. The more people you have, the more you stay pointed in the same direction.

Another drawback of large investments is the time they take. The time required to raise money grows with the amount. [7](Not linearly of course, or it would take forever to raise five million dollars. In practice it just feels like it takes forever.

Though if you include the cases where VCs don't invest, it would literally take forever in the median case. And maybe we should, because the danger of chasing large investments is not just that they take a long time. That's the best case. The real danger is that you'll expend a lot of time and get nothing.) When the amount rises into the millions, investors get very cautious. VCs never quite say yes or no; they just engage you in an apparently endless conversation. Raising VC scale investments is thus a huge time sink — more work, probably, than the startup itself. And you don't want to be spending all your time talking to investors while your competitors are spending theirs building things.

We advise founders who go on to seek VC money to take the first reasonable deal they get. If you get an offer from a reputable firm at a reasonable valuation with no unusually onerous terms, just take it and get on with building the company. [8](Some VCs will offer you an artificially low valuation to see if you have the balls to ask for more. It's lame that VCs play such games, but some do. If you're dealing with one of those you should push back on the valuation a bit.) Who cares if you could get a 30% better deal elsewhere? Economically, startups are an all-or-nothing game. Bargain-hunting among investors is a waste of time.`,
        korean: `13. 너무 많은 자금 조달

너무 적은 돈이 당신을 죽일 수 있다는 것은 분명하지만, 너무 많은 돈을 갖는 것과 같은 것이 있을까요?

예, 아니오. 문제는 돈 자체라기보다는 돈과 함께 오는 것입니다. Y Combinator에서 연설한 한 VC는 "내 돈 수백만 달러를 받으면 시계가 똑딱거리기 시작합니다."라고 말했습니다. VC가 당신에게 자금을 지원한다면, 그들은 당신이 돈을 은행에 넣어두고 라면으로 생활하는 두 사람으로 계속 운영하도록 내버려 두지 않을 것입니다. 그들은 그 돈이 일하게 되기를 원합니다. [6](사람들이 때때로 우리를 VC라고 부르기 때문에 덧붙이자면, 우리는 VC가 아닙니다. VC는 다른 사람들의 많은 돈을 투자합니다. 우리는 엔젤 투자자처럼 우리 자신의 적은 돈을 투자합니다.) 최소한 적절한 사무실 공간으로 옮기고 더 많은 사람을 고용하게 될 것입니다. 그것은 분위기를 바꿀 것이고, 전적으로 더 나은 방향은 아닐 것입니다. 이제 대부분의 사람들은 창업자가 아닌 직원이 될 것입니다. 그들은 덜 헌신적일 것이고, 무엇을 해야 할지 지시받아야 할 것이며, 사내 정치에 참여하기 시작할 것입니다.

많은 돈을 조달하면 회사는 교외로 이사하고 아이를 갖게 됩니다(회사가 커지고 관료화되는 것을 비유적으로 표현).

아마도 더 위험한 것은, 일단 많은 돈을 받으면 방향을 바꾸기가 더 어려워진다는 것입니다. 초기 계획이 회사에 무언가를 파는 것이었다고 가정해 봅시다. VC 자금을 받은 후에는 이를 위해 영업 인력을 고용합니다. 이제 기업 대신 소비자를 위해 이것을 만들어야 한다는 것을 깨달으면 어떻게 될까요? 그것은 완전히 다른 종류의 판매입니다. 실제로 일어나는 일은 당신이 그것을 깨닫지 못한다는 것입니다. 사람이 많을수록 같은 방향을 더 많이 가리키게 됩니다.

거액 투자의 또 다른 단점은 시간이 많이 걸린다는 것입니다. 자금 조달에 필요한 시간은 금액에 따라 증가합니다. [7](물론 선형적으로 증가하는 것은 아닙니다. 그렇지 않으면 5백만 달러를 모으는 데 영원히 걸릴 것입니다. 실제로는 영원히 걸리는 것처럼 느껴질 뿐입니다. 하지만 VC가 투자하지 않는 경우를 포함하면 중앙값의 경우 말 그대로 영원히 걸릴 것입니다. 그리고 아마도 그렇게 해야 할 것입니다. 왜냐하면 거액 투자를 쫓는 위험은 단지 시간이 오래 걸린다는 것뿐만이 아니기 때문입니다. 그것은 최선의 경우입니다. 진짜 위험은 많은 시간을 쏟고 아무것도 얻지 못할 수 있다는 것입니다.) 금액이 수백만 달러로 늘어나면 투자자들은 매우 신중해집니다. VC는 예 또는 아니오라고 명확하게 말하지 않고, 겉보기에 끝없는 대화에 당신을 참여시킵니다. 따라서 VC 규모의 투자를 유치하는 것은 엄청난 시간 낭비이며, 아마도 스타트업 자체보다 더 많은 작업일 것입니다. 그리고 경쟁자들이 제품을 만드는 데 시간을 보내는 동안 투자자들과 이야기하는 데 모든 시간을 보내고 싶지는 않을 것입니다.

VC 자금을 구하려는 창업자들에게는 합리적인 첫 번째 제안을 받으라고 조언합니다. 평판 좋은 회사로부터 합리적인 가치 평가와 특별히 부담스러운 조건 없이 제안을 받으면 그냥 받아들이고 회사 구축을 계속하십시오. [8](일부 VC는 당신이 더 많은 것을 요구할 배짱이 있는지 확인하기 위해 인위적으로 낮은 가치 평가를 제안할 것입니다. VC가 그런 게임을 하는 것은 유감스러운 일이지만, 일부는 그렇게 합니다. 그런 VC와 거래하고 있다면 가치 평가에 대해 약간 반박해야 합니다.) 다른 곳에서 30% 더 나은 거래를 할 수 있다면 누가 신경 쓰겠습니까? 경제적으로 스타트업은 전부 아니면 전무 게임입니다. 투자자들 사이에서 거래를 찾는 것은 시간 낭비입니다.`,
    },
    {
        english: `14. Poor Investor Management

As a founder, you have to manage your investors. You shouldn't ignore them, because they may have useful insights. But neither should you let them run the company. That's supposed to be your job. If investors had sufficient vision to run the companies they fund, why didn't they start them?

Pissing off investors by ignoring them is probably less dangerous than caving in to them. In our startup, we erred on the ignoring side. A lot of our energy got drained away in disputes with investors instead of going into the product. But this was less costly than giving in, which would probably have destroyed the company. If the founders know what they're doing, it's better to have half their attention focused on the product than the full attention of investors who don't.

How hard you have to work on managing investors usually depends on how much money you've taken. When you raise VC-scale money, the investors get a great deal of control. If they have a board majority, they're literally your bosses. In the more common case, where founders and investors are equally represented and the deciding vote is cast by neutral outside directors, all the investors have to do is convince the outside directors and they control the company.

If things go well, this shouldn't matter. So long as you seem to be advancing rapidly, most investors will leave you alone. But things don't always go smoothly in startups. Investors have made trouble even for the most successful companies. One of the most famous examples is Apple, whose board made a nearly fatal blunder in firing Steve Jobs. Apparently even Google got a lot of grief from their investors early on.`,
        korean: `14. 서투른 투자자 관리

창업자로서 투자자를 관리해야 합니다. 유용한 통찰력을 가지고 있을 수 있으므로 무시해서는 안 됩니다. 하지만 회사를 운영하도록 내버려 두어서도 안 됩니다. 그것은 당신의 일입니다. 투자자들이 자금을 지원하는 회사를 운영할 만큼 충분한 비전을 가지고 있다면 왜 그들이 직접 시작하지 않았을까요?

투자자를 무시하여 화나게 하는 것은 그들에게 굴복하는 것보다 덜 위험할 것입니다. 우리 스타트업에서는 무시하는 쪽으로 실수를 저질렀습니다. 제품에 투자하는 대신 투자자와의 분쟁으로 많은 에너지가 소모되었습니다. 하지만 이것은 굴복하는 것보다 비용이 적게 들었고, 굴복했다면 아마 회사를 망쳤을 것입니다. 창업자들이 자신이 무엇을 하고 있는지 안다면, 그렇지 않은 투자자들의 전적인 관심보다 제품에 절반의 관심을 집중하는 것이 더 낫습니다.

투자자 관리에 얼마나 많은 노력을 기울여야 하는지는 일반적으로 얼마나 많은 돈을 받았는지에 따라 다릅니다. VC 규모의 자금을 조달하면 투자자들은 상당한 통제권을 갖게 됩니다. 그들이 이사회 과반수를 차지하면 말 그대로 당신의 상사입니다. 창업자와 투자자가 동등하게 대표되고 중립적인 외부 이사가 결정 투표를 하는 더 일반적인 경우에는 모든 투자자가 외부 이사를 설득하기만 하면 회사를 통제할 수 있습니다.

일이 잘 풀리면 이것은 문제가 되지 않아야 합니다. 빠르게 발전하는 것처럼 보이면 대부분의 투자자는 당신을 내버려 둘 것입니다. 하지만 스타트업에서는 일이 항상 순조롭게 진행되는 것은 아닙니다. 투자자들은 가장 성공적인 회사들에게도 문제를 일으켰습니다. 가장 유명한 예 중 하나는 애플인데, 이사회는 스티브 잡스를 해고하는 거의 치명적인 실수를 저질렀습니다. 구글조차도 초기에 투자자들로부터 많은 어려움을 겪었다고 합니다.`,
    },
    {
        english: `15. Sacrificing Users to (Supposed) Profit

When I said at the beginning that if you make something users want, you'll be fine, you may have noticed I didn't mention anything about having the right business model. That's not because making money is unimportant. I'm not suggesting that founders start companies with no chance of making money in the hope of unloading them before they tank. The reason we tell founders not to worry about the business model initially is that making something people want is so much harder.

I don't know why it's so hard to make something people want. It seems like it should be straightforward. But you can tell it must be hard by how few startups do it.

Because making something people want is so much harder than making money from it, you should leave business models for later, just as you'd leave some trivial but messy feature for version 2. In version 1, solve the core problem. And the core problem in a startup is how to create wealth (= how much people want something x the number who want it), not how to convert that wealth into money.

The companies that win are the ones that put users first. Google, for example. They made search work, then worried about how to make money from it. And yet some startup founders still think it's irresponsible not to focus on the business model from the beginning. They're often encouraged in this by investors whose experience comes from less malleable industries.

It is irresponsible not to think about business models. It's just ten times more irresponsible not to think about the product.`,
        korean: `15. (가상적인) 이익을 위해 사용자를 희생함

처음에 사용자가 원하는 것을 만들면 괜찮을 것이라고 말했을 때, 올바른 비즈니스 모델을 갖는 것에 대해 아무것도 언급하지 않았다는 것을 눈치챘을 것입니다. 돈을 버는 것이 중요하지 않기 때문이 아닙니다. 창업자들이 망하기 전에 회사를 처분할 희망으로 돈을 벌 가능성이 없는 회사를 시작하라고 제안하는 것이 아닙니다. 우리가 창업자들에게 처음에 비즈니스 모델에 대해 걱정하지 말라고 말하는 이유는 사람들이 원하는 것을 만드는 것이 훨씬 더 어렵기 때문입니다.

왜 사람들이 원하는 것을 만드는 것이 그렇게 어려운지 모르겠습니다. 간단해야 할 것 같습니다. 하지만 얼마나 적은 스타트업만이 그것을 해내는지 보면 그것이 어려워야 한다는 것을 알 수 있습니다.

사람들이 원하는 것을 만드는 것이 그것으로부터 돈을 버는 것보다 훨씬 어렵기 때문에, 사소하지만 지저분한 기능을 버전 2로 남겨두는 것처럼 비즈니스 모델은 나중으로 미루어야 합니다. 버전 1에서는 핵심 문제를 해결하십시오. 그리고 스타트업의 핵심 문제는 부를 창출하는 방법(= 사람들이 무언가를 얼마나 원하는가 x 그것을 원하는 사람들의 수)이지, 그 부를 돈으로 전환하는 방법이 아닙니다.

승리하는 회사는 사용자를 최우선으로 생각하는 회사입니다. 예를 들어 구글입니다. 그들은 검색이 작동하도록 만든 다음 그것으로부터 돈을 버는 방법에 대해 걱정했습니다. 그럼에도 불구하고 일부 스타트업 창업자들은 처음부터 비즈니스 모델에 집중하지 않는 것이 무책임하다고 여전히 생각합니다. 그들은 종종 덜 유연한 산업에서 경험을 쌓은 투자자들로부터 이러한 격려를 받습니다.

비즈니스 모델에 대해 생각하지 않는 것은 무책임합니다. 제품에 대해 생각하지 않는 것은 단지 열 배 더 무책임합니다.`,
    },
    {
        english: `16. Not Wanting to Get Your Hands Dirty

Nearly all programmers would rather spend their time writing code and have someone else handle the messy business of extracting money from it. And not just the lazy ones. Larry and Sergey apparently felt this way too at first. After developing their new search algorithm, the first thing they tried was to get some other company to buy it.

Start a company? Yech. Most hackers would rather just have ideas. But as Larry and Sergey found, there's not much of a market for ideas. No one trusts an idea till you embody it in a product and use that to grow a user base. Then they'll pay big time.

Maybe this will change, but I doubt it will change much. There's nothing like users for convincing acquirers. It's not just that the risk is decreased. The acquirers are human, and they have a hard time paying a bunch of young guys millions of dollars just for being clever. When the idea is embodied in a company with a lot of users, they can tell themselves they're buying the users rather than the cleverness, and this is easier for them to swallow. [9](Suppose YouTube's founders had gone to Google in 2005 and told them "Google Video is badly designed. Give us $10 million and we'll tell you all the mistakes you made." They would have gotten the royal raspberry. Eighteen months later Google paid $1.6 billion for the same lesson, partly because they could then tell themselves that they were buying a phenomenon, or a community, or some vague thing like that.

I don't mean to be hard on Google. They did better than their competitors, who may have now missed the video boat entirely.)

If you're going to attract users, you'll probably have to get up from your computer and go find some. It's unpleasant work, but if you can make yourself do it you have a much greater chance of succeeding. In the first batch of startups we funded, in the summer of 2005, most of the founders spent all their time building their applications. But there was one who was away half the time talking to executives at cell phone companies, trying to arrange deals. Can you imagine anything more painful for a hacker? [10](Yes, actually: dealing with the government. But phone companies are up there.) But it paid off, because this startup seems the most successful of that group by an order of magnitude.

If you want to start a startup, you have to face the fact that you can't just hack. At least one hacker will have to spend some of the time doing business stuff.`,
        korean: `16. 손 더럽히기를 원하지 않음

거의 모든 프로그래머는 코드를 작성하는 데 시간을 보내고 다른 사람이 돈을 추출하는 지저분한 사업을 처리하도록 하는 것을 선호합니다. 게으른 사람들뿐만이 아닙니다. 래리와 세르게이도 처음에는 그렇게 느꼈던 것 같습니다. 새로운 검색 알고리즘을 개발한 후 그들이 가장 먼저 시도한 것은 다른 회사에 그것을 사도록 하는 것이었습니다.

회사를 시작한다고? 웩. 대부분의 해커는 그냥 아이디어를 갖는 것을 선호합니다. 하지만 래리와 세르게이가 발견했듯이 아이디어 시장은 그다지 크지 않습니다. 아이디어를 제품에 구현하고 그것을 사용하여 사용자 기반을 늘릴 때까지는 아무도 아이디어를 신뢰하지 않습니다. 그러면 그들은 큰 돈을 지불할 것입니다.

아마도 이것이 바뀔 수도 있겠지만, 많이 바뀔 것 같지는 않습니다. 인수자를 설득하는 데 사용자만큼 좋은 것은 없습니다. 위험이 줄어드는 것뿐만이 아닙니다. 인수자는 인간이며, 똑똑함보다는 사용자를 구매한다고 스스로에게 말할 수 있으며, 이것이 그들이 받아들이기 더 쉽습니다. [9](아이러니하게도, 효과가 있을 수 있는 페이스북의 한 가지 변형은 대학생 전용 페이스북입니다.) 사용자를 유치하려면 아마도 컴퓨터에서 일어나서 몇 명 찾아야 할 것입니다. 불쾌한 일이지만, 스스로 할 수 있다면 성공할 가능성이 훨씬 더 커집니다. 2005년 여름에 우리가 자금을 지원한 첫 번째 스타트업 배치에서 대부분의 창업자는 모든 시간을 애플리케이션 구축에 보냈습니다. 하지만 휴대폰 회사 임원들과 이야기하고 거래를 성사시키려고 절반의 시간을 자리를 비운 사람이 한 명 있었습니다. 해커에게 이보다 더 고통스러운 일을 상상할 수 있습니까? [10](네, 사실 있습니다: 정부를 상대하는 것입니다. 하지만 전화 회사도 만만치 않습니다.) 하지만 그만한 가치가 있었습니다. 이 스타트업은 그 그룹에서 가장 성공적인 스타트업으로 보이며, 그 차이는 엄청납니다.

스타트업을 시작하고 싶다면 해킹만으로는 안 된다는 사실을 직시해야 합니다. 최소한 한 명의 해커는 사업적인 일을 하는 데 시간을 할애해야 합니다.`,
    },
    {
        english: `17. Fights Between Founders

Fights between founders are surprisingly common. About 20% of the startups we've funded have had a founder leave. It happens so often that we've reversed our attitude to vesting. We still don't require it, but now we advise founders to vest so there will be an orderly way for people to quit.

A founder leaving doesn't necessarily kill a startup, though. Plenty of successful startups have had that happen. [11](Many more than most people realize, because companies don't advertise this. Did you know Apple originally had three founders?) Fortunately it's usually the least committed founder who leaves. If there are three founders and one who was lukewarm leaves, big deal. If you have two and one leaves, or a guy with critical technical skills leaves, that's more of a problem. But even that is survivable. Blogger got down to one person, and they bounced back.

Most of the disputes I've seen between founders could have been avoided if they'd been more careful about who they started a company with. Most disputes are not due to the situation but the people. Which means they're inevitable. And most founders who've been burned by such disputes probably had misgivings, which they suppressed, when they started the company. Don't suppress misgivings. It's much easier to fix problems before the company is started than after. So don't include your housemate in your startup because he'd feel left out otherwise. Don't start a company with someone you dislike because they have some skill you need and you worry you won't find anyone else. The people are the most important ingredient in a startup, so don't compromise there.`,
        korean: `17. 창업자 간의 다툼

창업자 간의 다툼은 놀랍게도 흔합니다. 우리가 자금을 지원한 스타트업의 약 20%에서 창업자가 떠났습니다. 너무 자주 발생하여 우리는 베스팅에 대한 태도를 바꿨습니다. 여전히 요구하지는 않지만, 이제는 창업자들에게 베스팅하도록 조언하여 사람들이 질서 있게 그만둘 수 있도록 합니다.

하지만 창업자가 떠난다고 해서 반드시 스타트업이 망하는 것은 아닙니다. 많은 성공적인 스타트업에서 그런 일이 있었습니다. [11](대부분의 사람들이 생각하는 것보다 훨씬 많습니다. 회사들이 이것을 광고하지 않기 때문입니다. 애플에 원래 세 명의 창업자가 있었다는 것을 알고 계셨습니까?) 다행히도 보통 가장 헌신적이지 않은 창업자가 떠납니다. 세 명의 창업자가 있고 미지근했던 한 명이 떠나면 큰일이 아닙니다. 두 명이 있고 한 명이 떠나거나, 중요한 기술을 가진 사람이 떠나면 더 큰 문제입니다. 하지만 그것조차도 생존 가능합니다. 블로거는 한 명으로 줄었지만 다시 일어섰습니다.

제가 본 창업자 간의 분쟁 대부분은 누구와 회사를 시작하는지에 대해 더 신중했다면 피할 수 있었을 것입니다. 대부분의 분쟁은 상황 때문이 아니라 사람 때문입니다. 즉, 피할 수 없다는 의미입니다. 그리고 그러한 분쟁으로 상처를 입은 대부분의 창업자는 회사를 시작할 때 아마도 억눌렀던 불안감을 가지고 있었을 것입니다. 불안감을 억누르지 마십시오. 회사가 시작된 후보다 시작하기 전에 문제를 해결하는 것이 훨씬 쉽습니다. 그러니 하우스메이트가 소외감을 느낄까 봐 스타트업에 포함시키지 마십시오. 필요한 기술을 가지고 있고 다른 사람을 찾지 못할까 봐 걱정된다는 이유로 싫어하는 사람과 회사를 시작하지 마십시오. 사람은 스타트업에서 가장 중요한 요소이므로 거기서 타협하지 마십시오.`,
    },
    {
        english: `18. A Half-Hearted Effort

The failed startups you hear most about are the spectacular flameouts. Those are actually the elite of failures. The most common type is not the one that makes spectacular mistakes, but the one that doesn't do much of anything — the one we never even hear about, because it was some project a couple guys started on the side while working on their day jobs, but which never got anywhere and was gradually abandoned.

Statistically, if you want to avoid failure, it would seem like the most important thing is to quit your day job. Most founders of failed startups don't quit their day jobs, and most founders of successful ones do. If startup failure were a disease, the CDC would be issuing bulletins warning people to avoid day jobs.

Does that mean you should quit your day job? Not necessarily. I'm guessing here, but I'd guess that many of these would-be founders may not have the kind of determination it takes to start a company, and that in the back of their minds, they know it. The reason they don't invest more time in their startup is that they know it's a bad investment. [12](I'm not dissing these people. I don't have the determination myself. I've twice come close to starting startups since Viaweb, and both times I bailed because I realized that without the spur of poverty I just wasn't willing to endure the stress of a startup.)

I'd also guess there's some band of people who could have succeeded if they'd taken the leap and done it full-time, but didn't. I have no idea how wide this band is, but if the winner/borderline/hopeless progression has the sort of distribution you'd expect, the number of people who could have made it, if they'd quit their day job, is probably an order of magnitude larger than the number who do make it. [13](So how do you know whether you're in the category of people who should quit their day job, or the presumably larger one who shouldn't? I got to the point of saying that this was hard to judge for yourself and that you should seek outside advice, before realizing that that's what we do. We think of ourselves as investors, but viewed from the other direction Y Combinator is a service for advising people whether or not to quit their day job. We could be mistaken, and no doubt often are, but we do at least bet money on our conclusions.)

If that's true, most startups that could succeed fail because the founders don't devote their whole efforts to them. That certainly accords with what I see out in the world. Most startups fail because they don't make something people want, and the reason most don't is that they don't try hard enough.

In other words, starting startups is just like everything else. The biggest mistake you can make is not to try hard enough. To the extent there's a secret to success, it's not to be in denial about that.`,
        korean: `18. 미흡한 노력

가장 많이 듣는 실패한 스타트업은 화려하게 망한 경우입니다. 그것들은 사실 실패의 엘리트입니다. 가장 흔한 유형은 화려한 실수를 저지르는 것이 아니라 아무것도 하지 않는 것입니다. 즉, 본업을 하면서 부업으로 시작했지만 아무런 성과도 거두지 못하고 점차 포기하게 된 프로젝트이기 때문에 우리가 전혀 듣지 못하는 것입니다.

통계적으로 실패를 피하고 싶다면 가장 중요한 것은 본업을 그만두는 것 같습니다. 실패한 스타트업 창업자 대부분은 본업을 그만두지 않고, 성공한 스타트업 창업자 대부분은 그만둡니다. 스타트업 실패가 질병이라면 CDC는 사람들에게 본업을 피하라고 경고하는 게시판을 발행할 것입니다.

그렇다고 본업을 그만둬야 한다는 뜻일까요? 반드시 그런 것은 아닙니다. 추측이지만, 이러한 예비 창업자 중 다수는 회사를 시작하는 데 필요한 종류의 결단력이 없을 수 있으며, 마음속으로는 그것을 알고 있을 것이라고 생각합니다. 그들이 스타트업에 더 많은 시간을 투자하지 않는 이유는 그것이 나쁜 투자라는 것을 알기 때문입니다. [12](이 사람들을 폄하하는 것이 아닙니다. 저 자신도 그런 결단력이 없습니다. 비아웹 이후 두 번이나 스타트업을 시작할 뻔했지만, 가난이라는 자극 없이는 스타트업의 스트레스를 견딜 의지가 없다는 것을 깨닫고 두 번 모두 포기했습니다.)

또한 과감하게 전념했다면 성공할 수 있었지만 그렇지 못한 사람들도 있을 것이라고 추측합니다. 이 범위가 얼마나 넓은지는 모르겠지만, 승자/경계선/절망적인 진행이 예상되는 분포를 가지고 있다면, 본업을 그만두었다면 성공했을 사람들의 수는 성공한 사람들의 수보다 아마 훨씬 더 클 것입니다. [13](그렇다면 당신이 본업을 그만둬야 하는 범주에 속하는지, 아니면 아마도 더 큰 범주인 그만두지 말아야 하는 범주에 속하는지 어떻게 알 수 있을까요? 저는 이것이 스스로 판단하기 어렵고 외부의 조언을 구해야 한다고 말하려는 참이었는데, 그것이 바로 우리가 하는 일이라는 것을 깨달았습니다. 우리는 스스로를 투자자라고 생각하지만, 다른 관점에서 보면 Y Combinator는 사람들에게 본업을 그만둬야 할지 말아야 할지 조언하는 서비스입니다. 우리가 틀릴 수도 있고, 의심할 여지 없이 종종 틀리지만, 적어도 우리는 우리의 결론에 돈을 겁니다.)

만약 그것이 사실이라면, 성공할 수 있었던 대부분의 스타트업은 창업자들이 전력을 다하지 않기 때문에 실패합니다. 그것은 확실히 제가 세상에서 보는 것과 일치합니다. 대부분의 스타트업은 사람들이 원하는 것을 만들지 못하기 때문에 실패하며, 대부분이 그렇지 못한 이유는 충분히 노력하지 않기 때문입니다.

다시 말해, 스타트업을 시작하는 것은 다른 모든 것과 같습니다. 당신이 저지를 수 있는 가장 큰 실수는 충분히 노력하지 않는 것입니다. 성공의 비결이 있다면, 그것을 부정하지 않는 것입니다.`,
    },
];

export { date, essay_data, korean_title, title };
